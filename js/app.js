...
const upDownArrow = `<svg xmlns="http://www.w3.org/2000/svg" width="21" height="21" fill="black" ><path d="M9 17.586V3H7v14.586l-2.293-2.293-1.414 1.414L8 21.414l4.707-4.707-1.414-1.414L9 17.586zM20.707 7.293 16 2.586l-4.707 4.707 1.414 1.414L15 6.414V21h2V6.414l2.293 2.293 1.414-1.414z"/></svg>`
...
let isEditorFirst = localStorage.getItem("layout") === "true" || localStorage.getItem("layout") === null;
localStorage.setItem("layout", isEditorFirst.toString());

const changeLayoutBtn = document.getElementById('changeLayoutBtn');
changeLayoutBtn.innerHTML = isEditorFirst ? upDownArrow : leftRightArrow;
...
const changeOrientationBtn = document.getElementById('changeOrientationBtn');

// Initialize orientation button
function initializeOrientation() {
  changeLayoutBtn.innerHTML = isVertical ? upDownArrow : leftRightArrow;
  
  // Clear any existing classes first
  removingClasses();
  
  // Then add the correct classes based on the stored orientation
  addingClasses();
  
  // Make sure editor layout is updated
  if (window.editor) {
    window.editor.layout();
  }
}

function initializeLayoutbtn() {
  changeOrientationBtn.innerHTML = isVertical ? upDownArrow : leftRightArrow;
}
...
function initializeLayoutbtn() {
  if (isVertical) {
    changeLayoutBtn.innerHTML = upDownArrow;
  }else {
    changeLayoutBtn.innerHTML = leftRightArrow;
  }
}
...
document.addEventListener('mousemove', (e) => {
  ...
  const newEditorHeight = editorContainer.offsetHeight + (isEditorFirst? deltaY : (-deltaY));
  const newConsoleHeight = consoleContainer.offsetHeight + (isEditorFirst? (-deltaY) : deltaY);

  const deltaX = e.clientX - startX;

  const newEditorWidth = editorContainer.offsetWidth + (isEditorFirst? deltaX : (-deltaX));
  const newConsoleWidth = consoleContainer.offsetWidth + (isEditorFirst? (-deltaX) : deltaX);
